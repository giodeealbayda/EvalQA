...
public class MainActivity extends Activity {
	...
	private static StudentAnswer newStudent;
	private static ArrayList<StudentAnswer> studentList;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		studentList = new ArrayList<StudentAnswer>();
	...
	public void openNextPage(boolean isPrivate){
		...
		if(!isPrivate){
			intent.setClass(getBaseContext(), PubPrivCreateTopic.class);
			System.out.println(math.getTitle());
			intent.putExtra("math", math);
			intent.putExtra("fil", filipino);
			intent.putExtra("eng", english);
			SessionManager.getInstance().setDeviceName("teacher");
		}else{
			intent.setClass(getBaseContext(), PubPrivCreateTopic.class);
		}
	...
	OnClickListener useStudent = new OnClickListener() {
		...
		public void onClick(View arg0) {
			...
			SessionManager.getInstance().setDeviceName(n);
